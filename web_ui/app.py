    ...
    try:
        ...
        if WORKSPACE_ROOT.exists():
            for worker_dir in WORKSPACE_ROOT.glob("worker_*"):
                ...
                # スクリーンショットを確認
                screenshots = list(SCREENSHOTS_DIR.glob(f"{worker_id}_*.png")) if SCREENSHOTS_DIR.exists() else []
                latest_screenshot = max(screenshots, key=lambda p: p.stat().st_mtime).name if screenshots else None
...
async def get_logs(log_file: str, lines: int = 100) -> JSONResponse:
    ...
    try:
        ...
        try:
            ...
            if not (LOGS_DIR.resolve() in log_path.parents or log_path.parent == LOGS_DIR.resolve()):
                return JSONResponse(
...
async def get_screenshot(screenshot_file: str) -> FileResponse | JSONResponse:
    ...
    if ".." in screenshot_file or "/" in screenshot_file or "\\" in screenshot_file:
        return JSONResponse(
            {"error": "Invalid screenshot file name"},
            status_code=400
        )
    ...
    try:
        ...
        if screenshot_path.parent != SCREENSHOTS_DIR.resolve():
            return JSONResponse(
                {"error": "Invalid screenshot file path"},
                status_code=400
            )
    except (ValueError, OSError):
        return JSONResponse(
...
async def get_worker_output(worker_id: str, lines: int = 50) -> JSONResponse:
    ...
    try:
        ...
        if not worker_id.startswith("worker_") or ".." in worker_id or "/" in worker_id or "\\" in worker_id:
            return JSONResponse(
        ...
        try:
            ...
            if worker_dir.parent != WORKSPACE_ROOT.resolve():
                return JSONResponse(
                    {"error": "Invalid worker directory"},
                    status_code=400
                )
        except (ValueError, OSError):
            return JSONResponse(
